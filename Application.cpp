...
void Application::readTest() {
	...
	if ( par->getcurrtime() == TEST_TIME ) {
		...
		log->LOG(&mp2[number]->getMemberNode()->addr, "--------------------------------------------Test one done at time: %d", par->getcurrtime());
	}
	...
	if ( par->getcurrtime() == (TEST_TIME + FIRST_FAIL_TIME) ) {
		...
		failedOneNode = false;
		log->LOG(&mp2[number]->getMemberNode()->addr, "--------------------------------------------Test two done at time: %d", par->getcurrtime());
	}
	...
	if ( par->getcurrtime() >= (TEST_TIME + FIRST_FAIL_TIME + STABILIZE_TIME) ) {
		...
		if ( par->getcurrtime() == (TEST_TIME + FIRST_FAIL_TIME + STABILIZE_TIME) ) {
			...
			number = findARandomNodeThatIsAlive();
			cout<<"random node to fail "<<number<<endl;
			...
			mp2[number]->clientRead(it->first);
			log->LOG(&mp2[number]->getMemberNode()->addr, "--------------------------------------------Test three a done at time: %d", par->getcurrtime());
		}
		...
		if ( par->getcurrtime() == (TEST_TIME + FIRST_FAIL_TIME + STABILIZE_TIME + STABILIZE_TIME) ) {
			...
			mp2[number]->clientRead(it->first);
			log->LOG(&mp2[number]->getMemberNode()->addr, "--------------------------------------------Test three b done at time: %d", par->getcurrtime());
		}
	...
	if ( par->getcurrtime() == (TEST_TIME + FIRST_FAIL_TIME + STABILIZE_TIME + STABILIZE_TIME + LAST_FAIL_TIME ) ) {
		...
		// This read should fail since at least quorum nodes are not alive
		mp2[number]->clientRead(it->first);
		log->LOG(&mp2[0]->getMemberNode()->addr, "--------------------------------------------Test four done at time: %d", par->getcurrtime());
	}
	...
	if ( par->getcurrtime() == (TEST_TIME + FIRST_FAIL_TIME + STABILIZE_TIME + STABILIZE_TIME + LAST_FAIL_TIME ) ) {
		...
		log->LOG(&mp2[number]->getMemberNode()->addr, "--------------------------------------------Test five done at time: %d", par->getcurrtime());
	}
...
void Application::updateTest() {
	 ...
	 * Test 4: FAIL A NON-REPLICA. Test if value is read correctly in quorum number of nodes after a NON-REPLICA IS FAILED
	 */
	cout<<"test 4 time "<<TEST_TIME + FIRST_FAIL_TIME + STABILIZE_TIME + STABILIZE_TIME + LAST_FAIL_TIME <<endl;
	if ( par->getcurrtime() == (TEST_TIME + FIRST_FAIL_TIME + STABILIZE_TIME + STABILIZE_TIME + LAST_FAIL_TIME ) ) {
		// Step 4.a. Find a node that is alive
		number = findARandomNodeThatIsAlive();
	 ...
	 */
	cout<<"test 5 time "<<TEST_TIME + FIRST_FAIL_TIME + STABILIZE_TIME + STABILIZE_TIME + LAST_FAIL_TIME <<endl;
	if ( par->getcurrtime() == (TEST_TIME + FIRST_FAIL_TIME + STABILIZE_TIME + STABILIZE_TIME + LAST_FAIL_TIME ) ) {
		string invalidKey = "invalidKey";
